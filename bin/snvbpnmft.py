...
if len(sys.argv) < 5:
    print "Usage: snvbpnmft.py <output directory> <number of strains> " \
          "<reference fasta> <sample 1 bam> <sample 2 bam> .. [sample N bam]"
    sys.exit(-1)
...
for i in range(num_samples):
    ...
    for line in vcf_file:
        ...
        if locus not in depths[alphabet[0]]:
            for a in alphabet:
                depths[a][locus] = dict()
...
beast_file.write("""
	...
	<!-- Define MCMC                                                             -->
	<mcmc id="mcmc" chainLength="2000000" autoOptimize="true" operatorAnalysis=""" + "\"" + os.path.join(out_dir,
                                                                                                         "aln.ops") + "\">" + """
		...
		<logTree id="treeFileLog" logEvery="1000" nexusFormat="true" fileName=""" + "\"" + os.path.join(out_dir,
                                                                                                        "aln.trees") + "\"" + """ sortTranslationTable="true">
			<treeModel idref="treeModel"/>
