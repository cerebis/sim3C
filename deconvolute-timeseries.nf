...
process WGS_Reads {
    ...
    if (params.debug) {
        ...
        echo "metaART.py -C gzip --profile $comm_prof -z $ms.options.num_samples -M $xfold -S ${key['seed']} \
                ...
                --coverage-out ${key}.cov -n ${key}.wgs $comm_seq ." > ${key}.wgs.r1.fq.gz

        echo "metaART.py -C gzip --profile $comm_prof -z $ms.options.num_samples -M $xfold -S ${key['seed']} \
                -s $ms.options.wgs.ins_std -m $ms.options.wgs.ins_len -l $ms.options.wgs.read_len \
                --coverage-out ${key}.cov -n ${key}.wgs $comm_seq ." > ${key}.wgs.r2.fq.gz
...
(wgs_out, map_in) = wgs_out.into(2)
    ...
    // just R1, R2 and reference
map_in = map_in.map{ it.pick(1, 2, 3) }
...
process WGSMap {
    ...
    input:
    set key, file(reads1), file(reads2), file(ref) from wgsmap_in

    output:
    set key, file("${key}.wgs2ref.bam"), file(reads1), file(reads2), file(ref) into wgsmap_out
